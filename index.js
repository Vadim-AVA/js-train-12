...
function createDictionary(entries) {
  ...
  entries.forEach((elem) => newmap.set(elem[0], elem[1]));
  return newmap;
} 
...
const myObject = {};
map.forEach((value, key) => myObject[key] = value);
return myObject; 
}
...
let newObj = Object();
newObj.dictionary = dictionary;
newObj.added = added;
newObj.rejected = rejected;
...
const iterator1 = dictionary.keys();
for (const iteratorkey of dictionary) {
  keys.push(iterator1.next().value);
};

const iterator2 = dictionary.values();
for (const iteratorkey of dictionary) {
  values.push(iterator2.next().value);
};
  
const iterator3 = dictionary.entries();
for (const iteratorkey of dictionary) {
  entries.push(iterator3.next().value);
};

let newObj = Object();
newObj.keys = keys;
newObj.values = values;
newObj.entries = entries;
...
const sortableArr = Object.entries(obj);
...
for (let [value] of set.entries()) {
  newmap.set(value, value.charCodeAt());
}
return newmap;
}
...
let someSet = new Set(["apple", "banana", "", "2"]);
